-- Setup Generation Stages for MessageCraft
-- Run this in your Supabase SQL Editor to enable real-time progress tracking

-- Create generation stages table (if not exists)
...
-- RLS Policies
DROP POLICY IF EXISTS "Users can view own generation stages" ON generation_stages;
...
DROP POLICY IF EXISTS "Service role can manage generation stages" ON generation_stages;
...
DROP TRIGGER IF EXISTS update_generation_stages_updated_at ON generation_stages;
